...
import bonesman.util.GeneralUtils;
import bonesman.util.KeywordInfo;
import bonesman.util.Sounds;
import bonesman.util.TextureLoader;
...
@SpireInitializer
public class BonesmanMod implements
        EditCardsSubscriber,
    ...
    public static void initialize() {
        ...
        BonesmanCharacter.Meta.registerColor();
    }
    ...
    private static String checkResourcesPath() {
        String name = BonesmanMod.class.getName(); //getPackage can be iffy with patching, so class name is used instead.
        ...
        if (!resources.exists()) {
            throw new RuntimeException("\n\tFailed to find resources folder; expected it to be at  \"resources/" + name + "\"." +
                    ...
                    "\t\"private static final String resourcesFolder = checkResourcesPath();\"\n" +
                    "\tat the top of the " + BonesmanMod.class.getSimpleName() + " java file.");
        }
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo)->{
            ...
            return initializers.contains(BonesmanMod.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
    ...
    public void receiveEditCharacters() {
        BonesmanCharacter.Meta.registerCharacter();
    }
